...
WHERE cl.countrycode = 'VAT' AND cl.isofficial = TRUE;
...
GROUP BY co.name
HAVING COUNT(*) = 1 AND MAX(cl.percentage) = 100.0;
...
FROM cities ci
WHERE ci.countrycode = 'SMR' AND ci.name != 'San Marino';
...
FROM cities ci
JOIN countries co ON ci.countrycode = co.code
WHERE co.continent = 'South America' AND (ci.name LIKE 'Serra%' OR ci.name LIKE 'Serra%');
...
SELECT co.capital, ci.name AS capital_name
FROM countries co
...
SELECT name AS city_name, countrycode, district, population
FROM cities
